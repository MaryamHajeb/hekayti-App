...
class SliedBloc extends Bloc<SliedEvent, SliedState> {
  ...
  SliedBloc({required this.repository}) : super(SliedInitial());
  @override
  Stream<SliedState> mapEventToState(SliedEvent event) async* {
    if (event is GetAllSlied) {
      yield SliedLoading();
      final failureOrData = await repository.getAllSlied(
          story_id: event.story_id, tableName: event.tableName);
